...
def init_db() -> None:
    ...
    # Seed products (minimal demo, admin keyin o‘zgartiradi)
    cur.execute("SELECT COUNT(*) AS c FROM products")
    if cur.fetchone()["c"] == 0:
        # Unsplash source URL: internetdan avtomatik rasm (kalit so'zga qarab)
        demo = [
...
def is_admin(user_id: int) -> bool:
    return user_id in ADMIN_IDS if ADMIN_IDS else False  # ADMIN_IDS bo‘sh bo‘lsa hech kim admin emas
...
def kb_cart(user_id: int) -> InlineKeyboardMarkup:
    ...
    for it in items[:8]:
        ...
        rows.append([
            InlineKeyboardButton(f"➖", callback_data=f"SUB:{pid}"),
            InlineKeyboardButton(f"{it['name']} x{it['qty']}", callback_data=f"SHOW:{pid}"),
            InlineKeyboardButton(f"➕", callback_data=f"ADD:{pid}"),
        ])
    if items:
...
# ================== BOT LOGIC ==================
STATE_ADD_PRODUCT = "ADD_PRODUCT"        # admin: step-by-step
STATE_SET_PRICE = "SET_PRICE"            # admin
STATE_CHECKOUT_PHONE = "CHECKOUT_PHONE"  # user
...
async def show_home(query_or_msg, uid: int):
    ...
    if hasattr(query_or_msg, "edit_message_text"):
        await query_or_msg.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=kb_main(is_admin(uid)))
    else:
...
async def show_categories(query):
    await query.edit_message_text("🛒 Katalog bo‘limlari:", reply_markup=kb_categories())

async def show_products(query, category_id: int):
    prods = get_products(category_id, only_active=True)
    if not prods:
        await query.edit_message_text("Bu bo‘limda hozircha mahsulot yo‘q.", reply_markup=kb_categories())
        return

    # 1 chi mahsulotni ko'rsatamiz, qolganiga navigatsiya uchun ro'yxat
    rows = []
    for p in prods[:10]:
    ...
    await query.edit_message_text("Mahsulotlar:", reply_markup=InlineKeyboardMarkup(rows))
...
async def show_product(query, uid: int, pid: int):
    ...
    # edit_message_media bo'lmasa oddiy textga qaytamiz
    try:
        if photo:
            ...
            await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=kb_product(pid))
    except Exception:
        await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=kb_product(pid))
...
async def show_cart(query, uid: int):
    ...
    if not items:
        await query.edit_message_text("🧺 Savatcha bo‘sh.", reply_markup=kb_cart(uid))
        return
    lines = []
    for it in items:
    ...
    await query.edit_message_text(text, parse_mode=ParseMode.HTML, reply_markup=kb_cart(uid))
...
async def checkout_start(query, context: ContextTypes.DEFAULT_TYPE, uid: int):
    ...
    await query.edit_message_text("📞 Telefon raqamingizni yuboring (masalan: +9665xxxxxxx):")
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == STATE_CHECKOUT_NOTE:
        ...
        await update.message.reply_text(f"✅ Buyurtma qabul qilindi! ID: <b>{oid}</b>\nTez orada aloqaga chiqamiz.",
        ...
        if ADMIN_IDS:
            ...
            lines = [f"🆕 <b>Yangi buyurtma</b> #{oid}",
    ...
    if state == STATE_ADD_PRODUCT:
        # format: category_id | name | description | photo_url
        # price keyin alohida set qilinadi
        txt = update.message.text.strip()
        parts = [p.strip() for p in txt.split("|")]
        if len(parts) < 2:
            await update.message.reply_text("Format xato.\nMisol:\n<code>1 | Pomidor | 1 kg | https://...</code>", parse_mode=ParseMode.HTML)
            return
        try:
            cat_id = int(parts[0])
        except:
    ...
    if state == STATE_BROADCAST:
        ...
        # oddiy broadcast: oxirgi buyurtma berganlar + savatchaga kirganlar bazasi yo'q,
        # shuning uchun minimal: faqat admin o'zi biladigan chatlarga forward qiladi.
        await update.message.reply_text("📣 Broadcast matni tayyor. Uni o‘zingiz kanal/guruhga yuboring.\n(Avto-broadcast uchun user bazasi kengaytiriladi.)")
        return
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("ADD:"):
        pid = int(data.split(":")[1])
        cart_add(uid, pid, 1)
        await query.answer("Qo‘shildi ✅", show_alert=False)
    ...
    if data == "CLEARCART":
        cart_clear(uid)
        await query.edit_message_text("🧹 Savatcha tozalandi.", reply_markup=kb_main(is_admin(uid)))
        return
    ...
    if data == "CONTACT":
        ...
        await query.edit_message_text(text, reply_markup=kb_main(is_admin(uid)))
        return
    ...
    if data == "ADMIN":
        if not is_admin(uid):
            ...
            return
        await query.edit_message_text("🛠 Admin panel", reply_markup=kb_admin())
        return

    if data == "A_ADD":
        if not is_admin(uid):
            return
        # category list show with ids
        cats = get_categories()
        lines = ["➕ Mahsulot qo‘shish", "", "Avval category_id ni tanlang (pastda):"]
        for c in cats:
            lines.append(f"• <b>{c['id']}</b> — {c['name']}")
        lines.append("")
        lines.append("Keyin shu formatda yuboring:")
        lines.append("<code>category_id | nomi | tavsif | foto_url</code>")
        lines.append("Misol:")
        lines.append("<code>1 | Pomidor | 1 kg | https://source.unsplash.com/800x600/?tomatoes</code>")
    ...
    if data == "A_ORDERS":
        ...
        if not orders:
            await query.edit_message_text("Buyurtmalar yo‘q.", reply_markup=kb_admin())
            return
...
def webhook():
    # Render health check ba’zan shu yo‘lga urishi mumkin
